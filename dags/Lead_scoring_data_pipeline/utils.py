...
from Lead_scoring_data_pipeline.mapping.significant_categorical_level import list_medium, list_platform, list_source
...
def load_data_into_db():
    ...
    conn = None
    try:
        df_lead_scoring = None
        conn = sqlite3.connect(DB_PATH+DB_FILE_NAME)
        df_lead_scoring = pd.read_csv(f"{DATA_DIRECTORY}/leadscoring.csv", index_col=[0])
        df_lead_scoring['total_leads_droppped'].fillna(0, inplace=True)
        df_lead_scoring['referred_lead'].fillna(0, inplace=True)
        df_lead_scoring.to_sql(name="loaded_data", con=conn, if_exists='replace', index=False)
        print("Loaded the data into DB successfully")
    except Exception as e:
        print(e)
        print("Failed to Load the data into DB")
    finally:
        if conn:
            conn.close()
...
def map_city_tier():
    ...
    df_lead_scoring["city_tier"].fillna(3.0, inplace=True)
...
def map_categorical_vars():
    ...
    df_lead_scoring.loc[~df_lead_scoring['first_platform_c'].isin(list_platform), 'first_platform_c'] = 'others'
    df_lead_scoring.loc[~df_lead_scoring['first_utm_medium_c'].isin(list_medium), 'first_utm_medium_c'] = 'others'
    df_lead_scoring.loc[~df_lead_scoring['first_utm_source_c'].isin(list_source), 'first_utm_source_c'] = 'others'
    df_lead_scoring.to_sql(name="categorical_variables_mapped", con=conn, if_exists='replace', index=False)
    conn.close()
...
def interactions_mapping():
    ...
    df_unpivot = pd.melt(df_lead_scoring, id_vars=index_variable, var_name='interaction_type', value_name='interaction_value')
